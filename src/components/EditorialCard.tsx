...
export function EditorialCard({
  ...
  return (
    <Link href={href} className={`group relative block ${ratio} overflow-hidden rounded-[18px] photo-placeholder`}>
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-petrol-950/90 via-petrol-950/25 to-petrol-950/5" />
      ...
      <div className="absolute inset-x-0 bottom-0 p-5 sm:p-6">
        ...
        {subtitle && <p className="mt-1.5 line-clamp-1 font-serif text-[14px] font-light italic text-cream/75">{subtitle}</p>}
      </div>
    </Link>
  );
}
